...
# 5. Sign the User CSR using the Root CA
Write-Host "`n--- Signing Certificate with CA ---"
& $openssl x509 -req -in user.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out user.crt -days 365 -extfile $confFile -extensions smime_ext

# 6. Package user cert & key as a PKCS#12 (PFX) file
Write-Host "`n--- Packaging into PFX ---"
& $openssl pkcs12 -export -out user.pfx -inkey user.key -in user.crt -certfile ca.crt -passout "pass:$Password"

Write-Host "`n======================================================="
Write-Host "S/MIME Test Certificate generation complete!"
Write-Host "Files generated in: $scriptDir"
Write-Host "  - ca.crt        : Trust this certificate in your client/OS to trust the user cert"
Write-Host "  - user.crt      : User public certificate"
Write-Host "  - user.key      : User private key"
Write-Host "  - user.pfx      : Combined certificate + key (password: '$Password')"
Write-Host "                    Import this file into Outlook/Thunderbird/etc."
Write-Host "======================================================="
